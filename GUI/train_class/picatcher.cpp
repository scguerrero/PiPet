...
// ═════════════════════════════════════════════════════════════
//  piCatcher  –  outer widget
// ═════════════════════════════════════════════════════════════

piCatcher::piCatcher(PiPet* pet, QWidget* parent, int canvasW, int canvasH)
    ...
    m_canvasW(canvasW), m_canvasH(canvasH)
{
    ...
    connect(canvas, &SnackCanvas::clicked, this, &piCatcher::onCanvasClicked);
    ...
    connect(btnRestart, &QPushButton::clicked, this, &piCatcher::onRestart);
...
void piCatcher::keyPressEvent(QKeyEvent* e)
{
...
void piCatcher::keyReleaseEvent(QKeyEvent* e)
{
    if (e->isAutoRepeat()) return;
    switch (e->key()) {
...
void piCatcher::onCanvasClicked()
{
    if (m_state == GState::Idle || m_state == GState::Dead)
        startGame();
}

void piCatcher::onRestart() { startGame(); }

// ═════════════════════════════════════════════════════════════
//  Timer → tick
// ═════════════════════════════════════════════════════════════

void piCatcher::timerEvent(QTimerEvent* e)
{
...
void piCatcher::addParticles(float x, float y, const QColor& col, int n)
{
    for (int i = 0; i < n; ++i) {
...
bool piCatcher::overlaps(float ax, float ay, float aw, float ah,
