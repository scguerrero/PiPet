/* Author: Luke Cerwin
 * Fixed: celebratePetBirthday() now uses else if so Baby→Teen, not Baby→Adult
 */
#include "Player.h"
using namespace std;
...
void Player::celebratePetBirthday() {
    // FIX: was two separate ifs — Baby would skip Teen and go straight to Adult.
    // Now uses else if so only one transition happens per call.
    if (pet.age_group() == "Baby")
        pet.set_age_group("Teen");
    else if (pet.age_group() == "Teen")
        pet.set_age_group("Adult");
    // Adult stays Adult
}
